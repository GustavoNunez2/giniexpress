...
function f(t=c){const e=document.getElementById("productGrid"),o=document.getElementById("productCount");if(t.length===0){e.innerHTML=`
                ...
                <div class="card-minimal flex flex-col overflow-hidden fade-in" style="animation-delay: ${a*.02}s">
                    ...
                    <div class="p-6 flex flex-col flex-1">
                        <h4 class="font-bold text-slate-900 mb-1 leading-tight line-clamp-1">${n.titulo}</h4>
                        <p class="text-slate-400 text-xs mb-6 line-clamp-2 leading-relaxed">${n.descripcion||"Sin descripción"}</p>
                        
                        <div class="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between">
                            <span class="text-lg font-bold text-slate-900">$${(n.precio||0).toLocaleString()}</span>
                            
                            <div class="flex items-center gap-2">
                                <div class="qty-minimal flex items-center gap-2">
                                    <button onclick="event.stopPropagation(); changeQtyInGrid(${n.id}, -1)" class="w-6 h-6 flex items-center justify-center font-bold text-slate-400 hover:text-slate-900 transition-colors">-</button>
                                    <input type="number" id="qty-${n.id}" value="1" min="1" class="w-6 text-center text-xs font-bold border-none focus:ring-0 p-0 bg-transparent" onclick="event.stopPropagation()">
                                    <button onclick="event.stopPropagation(); changeQtyInGrid(${n.id}, 1)" class="w-6 h-6 flex items-center justify-center font-bold text-slate-400 hover:text-slate-900 transition-colors">+</button>
                                </div>
                                <button onclick="event.stopPropagation(); addToCart(${n.id}, parseInt(document.getElementById('qty-${n.id}').value))" class="btn-primary p-2.5 rounded-lg">
            ...
            `).join(""),lucide.createIcons()}function g(){const t=document.getElementById("searchInput").value.toLowerCase().trim(),e=c.filter(o=>o.titulo.toLowerCase().includes(t));f(e)}function r(){const t=document.getElementById("cartItems"),e=document.getElementById("cartTotal"),o=document.getElementById("cartCount");if(s.length===0){t.innerHTML=`
                ...
                `}).join(""),e.textContent=`$${n.toLocaleString()}`,o.textContent=a,o.classList.remove("hidden"),lucide.createIcons()}function y(t,e){const o=c.find(a=>a.id===t);if(!o)return;const n=s.find(a=>a.id===t);n?n.qty+=e:s.push({...o,qty:e}),u(),r(),P()}function L(t,e){const o=s.find(n=>n.id===t);o&&(o.qty+=e,o.qty<=0?x(t):(u(),r()))}function x(t){s=s.filter(e=>e.id!==t),u(),r()}function u(){localStorage.setItem("cart_pro",JSON.stringify(s))}function i(){const t=document.getElementById("cartDrawer"),e=document.getElementById("cartOverlay");t.classList.contains("closed")?(t.classList.replace("closed","open"),e.classList.replace("opacity-0","opacity-100"),e.classList.remove("pointer-events-none"),document.body.style.overflow="hidden"):(t.classList.replace("open","closed"),e.classList.replace("opacity-100","opacity-0"),e.classList.add("pointer-events-none"),document.body.style.overflow="auto")}function k(t){const e=c.find(n=>n.id===t);if(!e)return;document.getElementById("modalImg").src=e.imagen_url,document.getElementById("modalTitle").textContent=e.titulo,document.getElementById("modalPrice").textContent=`$${(e.precio||0).toLocaleString()}`,document.getElementById("modalDescription").textContent=e.descripcion||"Sin descripción detallada disponible.",document.getElementById("modalQty").value=1,document.getElementById("productModal").classList.replace("hidden","visible"),document.body.style.overflow="hidden",document.getElementById("modalAddBtn").onclick=()=>{y(e.id,parseInt(document.getElementById("modalQty").value)),d()}}function d(){document.getElementById("productModal").classList.replace("visible","hidden"),document.body.style.overflow="auto"}function S(t){const e=document.getElementById("modalQty");let o=parseInt(e.value)+t;o<1&&(o=1),e.value=o}function q(t,e){const o=document.getElementById(`qty-${t}`);let n=parseInt(o.value)+e;n<1&&(n=1),o.value=n}function v(){if(s.length===0)return alert("El carrito está vacío");let t=0,e=`Hola! Me interesa realizar el siguiente pedido:

`;s.forEach(n=>{const a=(n.precio||0)*n.qty;t+=a,e+=`*${n.qty}x* ${n.titulo} ($${(n.precio||0).toLocaleString()})
`}),e+=`
*Total:* $${t.toLocaleString()}`;const o=`https://wa.me/${m}?text=${encodeURIComponent(e)}`;window.open(o,"_blank")}function h(){const t="Hola, tengo una consulta sobre el catálogo...";document.getElementById("whatsappFloating").href=`https://wa.me/${m}?text=${encodeURIComponent(t)}`}function P(){const t=document.getElementById("openCart");t.classList.add("scale-110"),setTimeout(()=>t.classList.remove("scale-110"),300)}E();window.openProductDetail=k;window.updateModalQty=S;window.changeQtyInGrid=q;window.addToCart=y;window.toggleCart=i;window.closeModal=d;window.removeFromCart=x;window.updateCartQty=L;window.finalizeOrder=v;window.updateWhatsAppLink=h;window.handleSearch=g;
